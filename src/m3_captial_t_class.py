...
class CapitalT(object):
    ...
    def __init__(self, intersection_center, width, height, letter_thickness):
        ...
        halfwidth = width / 2
        halfthickness = letter_thickness / 2

        self.corner_hleft = rg.Point((intersection_center.x - halfwidth),
                                     (intersection_center.y - halfthickness))
        self.corner_hright = rg.Point((intersection_center.x + halfwidth),
                                      (intersection_center.y + halfthickness))
        self.corner_vleft = rg.Point((intersection_center.x - halfthickness),
                                     (intersection_center.y - halfthickness))
        self.corner_vright = rg.Point((intersection_center.x + halfthickness),
                                      (intersection_center.y + height -
                                       halfthickness))

        self.v_rect = rg.Rectangle(self.corner_vleft, self.corner_vright)
        self.h_rect = rg.Rectangle(self.corner_hleft, self.corner_hright)
    ...
    def clone(self):
        ...
        #     clone.pdf.
        # --------------------------------------------------------------
        rectangle = CapitalT(self.intersection_center, self.width,
                             self.height, self.letter_thickness)
        rectangle.set_colors(self.fill_color, self.outline_color)
